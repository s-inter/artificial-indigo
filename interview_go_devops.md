...
- Pointers, pass-by-value semantics
- Slices, arrays, maps: internals, capacity, growth, reference semantics

### Concurrency
- Goroutines: creation, scheduling, M:N threading model
- Channels: buffered vs unbuffered, direction, `select`, closing
- Common patterns: worker pools, pipelines, fan-in/fan-out
- Synchronization primitives: `sync.Mutex`, `sync.RWMutex`, `sync.WaitGroup`, `sync.Once`
- Race detection and avoiding data races
- Context package: cancellation, timeouts, value propagation

### Types & Interfaces
- Structs: composition, embedding, tags
...
- Testing: unit tests, table-driven tests, benchmarks, mocks
- Go tools: `go build`, `go test`, `go mod`, `go fmt`, `go vet`

### DevOps-Specific Topics
- Building CLIs with `flag` or third-party libraries (cobra, viper)
